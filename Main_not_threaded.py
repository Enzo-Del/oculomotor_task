...
class Camera():
    ...
    def __init__(self, cam_id, mice_name, res_width, res_height, FPS, grayscale):
        ...
        self.out = cv2.VideoWriter(r'C:\Users\taches-comportements\Downloads\oculomotor_task'+chr(92)+ str(today)+'_'+str(self.mice_name) +'.mp4', self.video_type, self.FPS, (self.res_width, self.res_height), grayscale)
...
class PosePrediction():
    ...
    def dispPredictionsv2(self, frame, counter, pupil_center, radius):
        ...
        image = cv2.drawMarker(image, (int(pupil_center[0][counter]), int(pupil_center[1][counter])),(0,0,255),cv2.MARKER_STAR, 5, 1)
...
class Gui(PeristalticPump):
    def __init__(self, com_port, water_spout_qty):
        ...
        tk.Label(self.root, text="Si tout va mal, c'est que tout va bien. MV").grid(row=25, column=0, sticky=W)
        ...
        self.go_button.grid(column=3, row=16)
        ...
        try :
            self.text_file = open(
                r"C:\Users\taches-comportements\Downloads\oculomotor_task" + chr(92) + str(date.today()) + '_' + str(
...
filename_V = r'C:\Users\taches-comportements\Pictures\Camera Roll\WIN_20210419_17_36_47_Pro.mp4'
#cfg = auxiliaryfunctions.read_config(r"C:\Users\taches-comportements\Desktop\TOM-net_v0.9-Enzo-2021-03-20\config.yaml")
modelfolder =(r"C:\Users\taches-comportements\Desktop\TOM_netv2-Enzo-2021-04-20\TOM_netv2-Enzo-2021-04-20\exported-models\DLC_TOM_netv2_mobilenet_v2_1.0_iteration-0_shuffle-1")
...
cam = Camera(filename_V, user_interface.mice_name, 640, 360,30,1)
...
while finished ==False:
    ...
    if probability :
        ...
        if azimutal_angle[counter] > 20 :
            azimutal_angle[counter] = math.nan
        ...
        if trial_started == True and (time.time()-trial_tmsp) < parameters["Answer window duration"] :
            ...
            #Saccade detection
                ...
                saccade_counter += 1
                print('seuil déclanché : {}'.format(variation_rate[counter]))
...
plt.xlabel('Time [s]')
plt.ylabel('Angle [degrees]')
plt.title(str(parameters['Mice name'] +' : variation rate'))
plt.grid()
plt.show()
